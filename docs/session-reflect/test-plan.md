...
验证 session-reflect 在真实会话中完整链路的正确性，包括冷启动初始化、证据提取、评分融合、context.md 生成、history.md 追加。
...
| 4 | AI 应对指南 | 不含分数，只有行为描述 |
| 5 | 综合画像 | 新增内容有会话证据支撑，无项目私有信息 |
| 6 | history.md | 追加了新条目，格式正确，含评分和依据 |
| 7 | 评分质量 | 依据引用具体行为，不是泛化评价 |
| 8 | 冷启动评分 | 综合分 = 本次评分（无 EMA 融合） |
