...
2. 会话分析阶段：提取 5 类用户级证据（沟通偏好、表达模式、协作偏好、工程偏好、决策与风险偏好），基于证据直接生成综合画像、AI 应对指南、偏好与习惯。
3. 写回阶段：覆盖写回 `context.md`（只含基本信息、综合画像、AI 应对指南、偏好与习惯），追加写入 `history.md`（含关键证据和变更记录）。
